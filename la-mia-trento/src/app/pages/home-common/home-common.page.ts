...
export class HomeCommonPage implements OnInit,AfterViewInit {
  // editMode = false;
  categories: any = [];
  // allCategories: any = [];
  elementsGallery: any[] ;
  ...
  if (await this.config.getModuleEntries())
  {this.categories = (await this.config.getModuleEntries()).map(x => this.convertCategories(x));
  }
  if (await this.config.getCarousel())
  {this.elementsGallery = (await this.config.getCarousel()).map(x => this.convertGallery(x));}
