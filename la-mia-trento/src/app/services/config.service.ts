...
export class ConfigService {
  ...
  getChoosen(): string {
    return localStorage.getItem(CHOOSEN_KEY)|| null;
  }
  ...
  setDefaultHome(type: any): Promise<any> {
    return new Promise<void>(async (resolve, reject) => {
      ...
      if (JSON.stringify(this.moduleEntries) === '{}') {
        const profile$ = this.http.get("assets/configuration/default-profiles-module.json");
  ...
  async getCarousel(): Promise<any> {
    ...
    else return await this.loadCarousel(); 
  }
  loadCarousel(): any {
  ...
  async getModuleEntries(): Promise<any> {
    if (JSON.stringify(this.moduleEntries) !== '{}')
    return this.moduleEntries;
    else return await this.loadModuleEntries(); 
  }
  loadModuleEntries(): any {
    if (JSON.stringify(this.moduleEntries) !== '{}' && undefined) {
